// https://www.acmicpc.net/problem/14245

package baekjoon.segmenttree;

import java.io.*;
import java.util.*;

public class XOR_14245 {

    private static int N, S, M;
    private static int[] arr, tree, propagate;

    private void solution() throws Exception {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(System.out));
        StringTokenizer st;

        N = Integer.parseInt(br.readLine());
        st = new StringTokenizer(br.readLine());
        arr = new int[N];
        for (int i = 0; i < N; ++i) {
            arr[i] = Integer.parseInt(st.nextToken());
        }

        M = Integer.parseInt(br.readLine());

        S = 1;
        while (S < N) {
            S <<= 1;
        }
        tree = new int[S * 2];
        propagate = new int[S * 2];

        initialize();

        int t, a, b, c;
        for (int i = 0; i < M; ++i) {
            st = new StringTokenizer(br.readLine());
            t = Integer.parseInt(st.nextToken());
            if (t == 1) {
                a = Integer.parseInt(st.nextToken());
                b = Integer.parseInt(st.nextToken());
                c = Integer.parseInt(st.nextToken());
                update(1, 0, S - 1, a, b, c);
            } else {
                a = Integer.parseInt(st.nextToken());
                bw.write(query(1, 0, S - 1, a) + "\n");
            }
        }

        bw.flush();
        bw.close();
        br.close();
    }

    private void initialize() {
        for (int i = 0; i < N; ++i) {
            tree[S + i] = arr[i];
        }

        for (int i = S - 1; i > 0; --i) {
            tree[i] = tree[i * 2] ^ tree[i * 2 + 1];
        }
    }

    private void update(int node, int left, int right, int updateLeft, int updateRight, int value) {
        lazyPropagate(node, left, right);
        if (updateRight < left || right < updateLeft) {
            return;
        }
        if (updateLeft <= left && right <= updateRight) {
            if ((right - left + 1) % 2 == 1) {
                tree[node] ^= value;
            }
            if (left != right) {
                propagate[node * 2] ^= value;
                propagate[node * 2 + 1] ^= value;
            }
        } else {
            int mid = (left + right) / 2;
            update(node * 2, left, mid, updateLeft, updateRight, value);
            update(node * 2 + 1, mid + 1, right, updateLeft, updateRight, value);
            tree[node] = tree[node * 2] ^ tree[node * 2 + 1];
        }
    }

    private int query(int node, int left, int right, int target) {
        lazyPropagate(node, left, right);
        if (target < left || right < target) {
            return 0;
        }
        if (left == right) {
            return tree[node];
        }
        int mid = (left + right) / 2;
        return query(node * 2, left, mid, target) ^
                query(node * 2 + 1, mid + 1, right, target);
    }

    private void lazyPropagate(int node, int left, int right) {
        if (propagate[node] != 0) {
            if ((right - left + 1) % 2 == 1) {
                tree[node] ^= propagate[node];
            }
            if (left != right) {
                propagate[node * 2] ^= propagate[node];
                propagate[node * 2 + 1] ^= propagate[node];
            }
            propagate[node] = 0;
        }
    }

    public static void main(String[] args) throws Exception {
        new XOR_14245().solution();
    }
}
