...
public class Wormhole {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (TC-- > 0) {
            ...
            boolean negativeCycle = bellmanFord(N, M, edges);
    ...
    private static boolean bellmanFord(int v, int e, Edge[] edges) {
        long[] distance = new long[v + 1];

        for (int i = 0; i < v; ++i) {
            boolean update = false;
            for (Edge edge: edges) {
                if (distance[edge.s] != INF && distance[edge.e] > distance[edge.s] + edge.time) {
